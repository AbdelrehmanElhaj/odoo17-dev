...
class MaintenanceResourceUtilization(models.Model):
    ...
    hourly_rate = fields.Float(string='Hourly Rate', compute='compute_hourly_rate', readonly=True)
    ...
    @api.depends('employee_id','employee_id.hourly_cost')
    def compute_hourly_rate(self):
        for record in self:
            employee = record.employee_id
            record.hourly_rate = employee and float(employee.hourly_cost) or 0.0
